 ...
 */
internals.getUser = function (authUtils, UserModel, userData) {
    var user = new UserModel();
    var formatQuery = user._utils.formatQuery;

    //clone to avoid mutation
    var username = userData.username;
    var qbObj = { orWhere: formatQuery(userData) };
    if (username) {
        qbObj.orWhere.username = authUtils.encryptUsername(username);
    }

    return user.query(qbObj).fetch();
};
 ...
 * @return {string}     JSON
 */
internals.callToJson = function (obj) {
    return obj.toJSON();
};
 ...
 * @return {Promise}
 */
internals.callFetch = function (obj) {
 ...
 */
internals.addUser = function (authUtils, UserModel, userData) {
    var userDataClone = _.clone(userData);
    var username = userDataClone.username;
    var password = userDataClone.password;
    var setHashedPassword = function setHashedPassword(password) {
    ...
    return authUtils.hashPassword(password).then(setHashedPassword).then(_.partialRight(_.defaults, internals.defaultValues)).then(saveUser);
};

module.exports.post = {
    tags: ['api', 'users'],
    notes: ['Creates a new user with the properties in the payload.'].join('<br>'),
    ...
    handler: function handler(request, reply) {
        var authUtils = request.server.plugins.utilities.auth;
        var User = request.server.plugins.bookshelf.model('User');
        var userData = {
         ...
         */
        var compareExistingUser = function compareExistingUser(existingUser) {
            if (!existingUser) {
            ...
            var existingProperties = _.reduce(userData, function (res, val, key) {
        ...
        internals.getUser(authUtils, User, userData).then(compareExistingUser).then(_.partial(internals.addUser, authUtils, User, request.payload)).then(internals.callFetch).then(internals.sanitize).then(internals.callToJson).then(reply)['catch'](function (err) {
