'use strict';
const Bluebird = require('bluebird');
const _ = require('lodash');
const mockConsortia = require('../../mocks/mock-consortia.json');
const controller = require('../../controllers/coinstac/consortia.js');

const assertIndexes = (db) => {
    return db.createIndex({
        index: {
            fields: ['sha']
        }
    }); // ToDo handle error condition
};

/**
 * Add mock consortia to DB if it is empty
 * @param  {object} db pouchDB instance
 * @return {Promise}    resolves when databases have been added, or immediately
 */
const addMockConsortia = (db) => {
    return db.all()
        .then((docs) => {
            let addPromises;
            if (docs.length === 0) {
                addPromises = _.map(mockConsortia, (consortium) => {
                    return db.add(consortium);
                });

                return Bluebird.all(addPromises);
            } else {
                return Bluebird.resolve();
            }
        });
};

const loadConsortiaDbs = (db, DbConstructor) => {
    return db.all()
        .then((docs) => {
            const dbPromises = _.map(docs, (consortium) => {
                const id = consortium._id;
                return controller.getConsortiaDb(id, DbConstructor);
            });

            if (dbPromises.length) {
                return Bluebird.all(dbPromises);
            } else {
                return Bluebird.resolve();
            }
        });
};

exports.register = (server, options, next) => {
    const path = '/consortia';

    const db = server.plugins.houchdb.consortiaMeta;
    const dbConstructor = server.plugins.houchdb.constructor;

    assertIndexes(db);

    server.route({
        method: 'GET',
        path: path,
        config: controller.getAll
    });

    server.route({
        method: 'GET',
        path: path + '/{id}',
        config: controller.getOne
    });

    server.route({
        method: 'POST',
        path: path,
        config: controller.post
    });

    server.route({
        method: 'PUT',
        path: path + '/{id}',
        config: controller.put
    });

    addMockConsortia(db)
        .then(_.partial(loadConsortiaDbs, db, dbConstructor))
        .then(_.noop)
        .then(next)
        .catch(next);
};

module.exports.register.attributes = {
    name: 'coinstac consoria routes'
};
