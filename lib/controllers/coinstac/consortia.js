...
const osr = require('coinstac-distributed-algorithm-set').oneShotRegression;
...
internals.watchConsortiumDb = (db, server) => {
    ...
    const calculateAverage = (values) => {
        ...
        // TODO: these keys should be defined in the consortium
        const roiKeys = _.keys(values[0]);
        const roiVectors = _.map(values, (roiObj) => {
            return _.map(roiKeys, (key)=> {
                return roiObj[key] || 0;
            });
        });

        const resultVector = osr.aggregateAvg(roiVectors);
...
internals.getConsortiumDb = (name, server) => {
    ...
    name = pouchWConfig.baseName + '-' + _.kebabCase(name.toLowerCase());
    if (_.find(internals.consortiaDBClients, { name: name })) {
        return _.find(internals.consortiaDBClients, { name: name });
    }
    ...
    internals.consortiaDBClients.push({ name: name, db: newDb });
    return newDb.info()
