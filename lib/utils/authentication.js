...
module.exports.register = function(server, options, next) {
    ...
    const relations = Bluebird.promisifyAll(server.plugins.relations);
    ...
    function refreshCurrentStudyRoles(username) {
        ...
        return new UserRole()
            ...
            .then((roleCollection) => {
                ...
                .then(() => {
                    return addRolesFromCollection(username, roleCollection);
                }).then(() => {
                    return relations
                        .studyAsync('Describe what %s can do', username)
                        .then((studyRoles) => {
                            return studyRoles;
                        });
                });
    ...
    function addRolesFromCollection(username, roleCollection) {
        let roleAdditions = [];
        roleCollection.each((value) => {
            roleAdditions.push(
                relations.studyAsync(
                    '%s is the %s of %s',
                    username,
                    value.related('role').get('label').toLowerCase(),
                    value.get('studyId')));
        });
